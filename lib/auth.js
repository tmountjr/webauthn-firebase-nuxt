...
export const registerCredential = ({ user, credentials }) => {
  ...
  const options = fido2.generateRegistrationOptions({
    rpName: 'Webauthn / Firebase / Nuxt Demo',
...
export const signinRequest = ({ user, credId }) => {
  try {
    ...
    for (const allow of options.allowCredentials) {
      allow.id = base64url.toBuffer(allow.id)
    }
    options.challenge = base64url.toBuffer(options.challenge)
...
export const signinResponse = async ({ user, rawCredential }) => {
  ...
  if (rawCredential.response) {
    const clientDataJSON = base64url.encode(rawCredential.response.clientDataJSON)
    const authenticatorData = base64url.encode(rawCredential.response.authenticatorData)
    const signature = base64url.encode(rawCredential.response.signature)
    const userHandle = base64url.encode(rawCredential.response.userHandle)

    credentialObject.response = {
      clientDataJSON,
      authenticatorData,
      signature,
      userHandle
